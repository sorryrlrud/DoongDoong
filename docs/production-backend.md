...
완전 삭제된 사용자의 기존 JWT가 아직 만료되지 않았더라도 서버는 Auth 계정 존재 여부를 확인해 `ACCOUNT_DELETED`로 거부합니다. 클라이언트는 이 응답에만 로컬 세션을 교체하고 새 익명 프로필의 온보딩을 시작하므로 삭제된 UUID가 다시 생성되지 않습니다.

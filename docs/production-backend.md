...
관리자 계정에는 GitHub identity가 연결되어야 합니다. 먼저 Supabase Authentication의 GitHub provider와 Manual Linking을 활성화한 뒤, 관리자 페이지 `#/admin`에서 GitHub 계정을 연결합니다. 연결이 끝난 뒤 표시되는 UID로 SQL Editor에서 다음 쿼리를 실행합니다.
